...
5. publish 결과는 `changesets/action`이 GitHub Release 자동 생성 — release note는 per-package CHANGELOG 기반.
6. publish 성공 직후 `release.yml`의 FF sync step (`if: steps.changesets.outputs.published == 'true'`) 이 `git push origin HEAD:main` 으로 main 을 현재 commit (= 방금 publish 된 dev tip) 으로 FF 동기화한다. main 이 npm latest 와 항상 정합되는 자동 mirror 로 유지되어 외부 stable pointer 역할을 한다 — 사람이 main 을 직접 push/PR 하지 않는다.
